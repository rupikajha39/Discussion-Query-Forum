...
export class InsertComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      this.http.get(BASE_URL + '/topic/subscribe/get/' + this.userId).subscribe(data => {
      this.topic = data;
    });

      this.http.get(BASE_URL + '/keyword').subscribe(data => {
      this.keyword = data;
    });
  ...
  addTopic(): void{
    ...
    if (this.newTopic !== undefined){
      ...
      this.http.post(BASE_URL + '/topic/create',
  ...
  addKeyword(): void{
    if (this.newKeyword !== undefined){
      const date = new Date;
      const dates = date.getFullYear() + '/' + date.getMonth() + '/' + date.getDay();
      this.http.post(BASE_URL + '/keyword/add',
    ...
    if (this.choosenKeyword !== undefined && this.choosenTopic !== undefined && this.question !== undefined){
        ...
        keywordQuestionList: choosenKeywordList
          ...
          if (data === 'true'){
            ...
            this.http.get(BASE_URL + '/notify/ques/'+this.choosenTopic.id+"/"+
